...
//   });
// };
// Job CRUD (adjusted createJob to remove company_logo)
...
//       `SELECT jp.*,
//               COUNT(ja.id) as application_count,
//               JSON_ARRAYAGG(JSON_OBJECT('id', ja.id, 'user_id', ja.user_id, 'status', ja.status)) as applications
//        FROM job_posts jp
//        LEFT JOIN job_applications ja ON jp.id = ja.job_post_id
//        WHERE jp.employer_id = ?
//        GROUP BY jp.id
...
// // FOR PREVIEW INDIVIDUAL PROFILE WHO APPLY THE JOB.
